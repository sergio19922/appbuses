...
useEffect(() => {
  ...
  if (lineasParam) {
    const items = lineasParam
      ...
      .map(l => {
        ...
        if (!pack || !routeId) return null; // 🚨 ignorar entradas inválidas
        return {
          key: `${pack}-${routeId}`,
          base: `/gtfs/${pack}`,
          routeId,
  ...
  // Mantener listado de carreteras dinámico
  useEffect(() => {
  ...
  // Selección de línea manual desde el panel
    ...
    setSelected(prev => {
      ...
      const color = '#F39400'; // 🔸 fijo en naranja
      ...
      return [
        ...prev,
        {
         ...
         nombrePersonalizado: item.nombrePersonalizado || "",  // 👈 añadir esto
          color,
  ...
  // 🔹 Agrupar seleccionadas por carretera
  ...
  onClick={() => {
    setSelected([]);
    navigate("/mapa", { replace: true }); // 🚀 limpia la URL
  }}
  style={{
    width: "100%",
    padding: 6,
...
>
  ...
  {s.nombrePersonalizado?.replace(/^\d+\.\s*/, "") ||
   ...
   s.shortName}
                      ...
                      <button
                        onClick={() => setSelected(prev => prev.filter(p => p.key !== s.key))}
                        style={{
                          marginLeft: 'auto',
...
<div
  style={{
    ...
    padding: "6px 10px",
    borderRadius: 6,
    backgroundColor: "#064e3b", // verde oscuro
    color: "#bbf7d0", // verde claro para contraste
    fontWeight: 600,
    textAlign: "center"
  }}
>
  Total seleccionadas: {selected.length}
      ...
      <section>
  <MapView
    lastSelected={selected[selected.length - 1]} // 🚀 la última seleccionada
